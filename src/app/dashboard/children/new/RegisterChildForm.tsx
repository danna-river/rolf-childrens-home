...
export function RegisterChildForm({ availableCountries }: Props) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      {/* Dynamic Navigation Frame Header */}
      ...
      {/* Persistent Process Timeline Track */}
      ...
      <div className="sticky bottom-0 bg-white border-t border-gray-100 px-4 py-4 mt-auto">
        {step < STEPS.length - 1 ? (
          <button onClick={() => setStep(s => s + 1)} disabled={!canNext()}
            className="w-full py-3.5 rounded-xl bg-blue-600 text-white font-semibold text-sm disabled:opacity-40 transition-opacity cursor-pointer">
            Continue to Next Step
          </button>
        ) : (
          <button onClick={handleSubmit} disabled={submitting}
            className="w-full py-3.5 rounded-xl bg-green-600 text-white font-semibold text-sm disabled:opacity-40 transition-opacity cursor-pointer">
            {submitting ? "Registering Child..." : "Register Child"}
          </button>
        )}
      </div>
    </div>
