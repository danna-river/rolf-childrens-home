...
export function RequestEditor({
  ...
  return (
    <div className="rounded-2xl border border-stone bg-white shadow-sm motion-safe:duration-200 motion-safe:ease-out motion-safe:animate-in motion-safe:fade-in-0 motion-safe:slide-in-from-bottom-2">
      <div className="flex flex-col gap-3 border-b border-stone px-5 py-5 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          <p className="text-sm font-bold uppercase tracking-wide text-navy/50">Request {index + 1}</p>
          <p className="mt-0.5 truncate text-lg font-semibold text-navy">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {canRemove && (
            <button
              ...
              className="inline-flex size-10 items-center justify-center rounded-xl text-red-700 motion-safe:transition-colors hover:bg-red-50 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-red-500"
            >
      ...
      <div className="space-y-4 px-5 py-5">
        {isSponsorship && (
          <div className="grid gap-3 lg:grid-cols-[minmax(0,1.35fr)_minmax(220px,0.65fr)]">
            <div className="space-y-3">
              ...
              <div className="max-h-72 overflow-y-auto rounded-2xl border border-stone bg-white">
                {filtered.map((child) => {
                  ...
                  return (
                    <button
                      ...
                      className={`grid w-full gap-2 border-b border-stone px-5 py-4 text-left last:border-b-0 motion-safe:transition motion-safe:duration-150 motion-safe:ease-out sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center ${
                        selected
                    ...
                    >
                      <span className="min-w-0">
                        ...
                        <span className="mt-1 flex flex-wrap gap-x-3 gap-y-1 text-base text-navy/55">
                      ...
                      <span className="flex items-center gap-2 sm:justify-end">
                        {child.id === longestWaitId && (
                          <span className="rounded-xl bg-amber-50 px-2.5 py-1.5 text-sm font-semibold text-amber-700">
                            Longest unmatched
                          </span>
                        )}
                        <span className={`flex size-7 items-center justify-center rounded-xl border ${selected ? 'border-teal bg-teal text-white' : 'border-stone bg-white text-navy/35'}`}>
                          {selected ? (
                ...
                {filtered.length === 0 && (
                  <div className="px-5 py-8 text-center text-base text-navy/50">
            ...
            <aside className="rounded-2xl border border-stone bg-ice p-4">
              <p className="text-sm font-bold uppercase tracking-wide text-navy/50">
                Selected
              </p>
              {selectedChildren.length === 0 ? (
                <p className="mt-2 text-base leading-6 text-navy/55">
                  No children selected for this request.
                </p>
              ) : (
                <div className="mt-2 space-y-2">
                  {selectedChildren.map((child) => (
                    <div key={child.id} className="flex items-center justify-between gap-2 rounded-xl bg-white px-3 py-2.5 text-base shadow-sm">
                      <span className="min-w-0">
                        <span className="block truncate font-semibold text-navy">{child.display_name}</span>
                        <span className="block truncate text-base text-navy/50">
                      ...
                      <button
                        ...
                        className="flex size-8 shrink-0 items-center justify-center rounded-xl text-navy/45 motion-safe:transition-colors hover:bg-red-50 hover:text-red-700 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-red-500"
                      >
        ...
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          ...
          {isSponsorship ? (
            ...
            <div className="rounded-xl border border-stone bg-ice px-4 py-3">
              <p className="text-base font-semibold text-navy/60">Record type</p>
        ...
        {isSponsorship && (
          <div className="inline-flex min-w-0 items-center gap-2 rounded-xl bg-ice px-4 py-3 text-base text-navy/65">
            <Clock3Icon className="size-4 shrink-0 text-teal" aria-hidden="true" />
