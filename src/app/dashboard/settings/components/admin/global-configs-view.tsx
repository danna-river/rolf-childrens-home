  ...
  if (showAddConfirm) {
    subActionArea = (
      <div className="bg-amber-50 border border-amber-100 rounded-xl p-3 flex items-center justify-between gap-4 animate-fade-in">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <button type="button" onClick={handleExecuteAppend} className="bg-amber-600 hover:bg-amber-700 text-white font-semibold text-xs px-3 py-1.5 rounded-lg cursor-pointer transition-all">
            Yes, Save Configuration
          </button>
          <button type="button" onClick={() => setShowAddConfirm(false)} className="bg-white text-gray-500 border border-gray-200 text-xs px-3 py-1.5 rounded-lg cursor-pointer hover:border-gray-300">
            Cancel
          </button>
        </div>
      </div>
    ...
    subActionArea = (
      <div className="bg-red-50 border border-red-100 rounded-xl p-3 flex items-center justify-between gap-4 animate-fade-in">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <button type="button" onClick={() => handleExecuteDelete(pendingDeleteCountry)} className="bg-red-600 hover:bg-red-700 text-white font-semibold text-xs px-3 py-1.5 rounded-lg cursor-pointer transition-all">
            Confirm Delete
          </button>
          <button type="button" onClick={() => setPendingDeleteCountry(null)} className="bg-white text-gray-500 border border-gray-200 text-xs px-3 py-1.5 rounded-lg cursor-pointer hover:border-gray-300">
            Cancel
          </button>
        </div>
      </div>
    ...
    subActionArea = (
      ...
      <form onSubmit={handleAddSubmitAttempt} className="flex flex-col sm:flex-row gap-2 max-w-xl">
        ...
        <input
          ...
          disabled={loading}
          required
          className="w-full sm:w-32 px-3 py-2 text-xs font-mono uppercase bg-white border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
        />
        <button type="submit" disabled={loading} className="bg-blue-600 hover:bg-blue-700 text-white font-semibold text-xs px-4 py-2 rounded-xl transition-all cursor-pointer shrink-0">
